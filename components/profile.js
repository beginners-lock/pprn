...
class Profile extends Component{
    ...
    async componentDidMount(){
        ...
        if(data){
            ...
            const defrequest = fetch(
                'https://ppbe01.onrender.com/getuserdatawithgames',//"http://localhost:3000/getuserdatawithgames",
                {
                    method: 'POST',
    ...
    saveChanges = (opt) => {
        if(opt==='fullname'){
            if(this.state.fullname!==''){
                ...
                const response = fetch(
                    'https://ppbe01.onrender.com/editfullname',//"http://localhost:3000/editfullname",
                    {
                        method: 'POST',
        ...
        if(opt==='username'){
            if(this.state.username!==''){
                ...
                const response = fetch(
                    'https://ppbe01.onrender.com/editusername',//"http://localhost:3000/editusername",
                    {
                        method: 'POST',
                        body: JSON.stringify(data),
                        headers: { 'Content-Type': 'application/json' }
        ...
        if(opt==='email'){
            if(this.state.email!==''){
                ...
                const response = fetch(
                    'https://ppbe01.onrender.com/verifyemail',//"http://localhost:3000/verifyemail",
                    {
                        method: 'POST',
                        body: JSON.stringify(data),
                        headers: { 'Content-Type': 'application/json' }
    ...
    verifycode = () => {
        ...
        if(this.state.digit1===''&&this.state.digit2===''&&this.state.digit3===''&&this.state.digit4===''&&this.state.digit5===''&&this.state.digit6===''){
            ...
            if(this.state.digit1===''||this.state.digit2===''||this.state.digit3===''||this.state.digit4===''||this.state.digit5===''||this.state.digit6===''){
                ...
                const response = fetch(
                    'https://ppbe01.onrender.com/verifyemailcode',//"http://localhost:3000/verifyemailcode",
                    {
                        method: 'POST',
                        body: JSON.stringify(data),
                        headers: { 'Content-Type': 'application/json' }
    ...
    profPicker = async () => {
        ...
        fetch(
            'https://ppbe01.onrender.com/profpicupload',//"http://localhost:3000/profpicupload",
            {
                method: 'POST',
                body: data
            }  
        ...
        return(
            <View style={{backgroundColor:colors[this.context.theme.mode].background, height:Dimensions.get('window').height}}>
                <View style={{display:'flex', flexDirection:'row', alignItems:'center', justifyContent:'flex-start', marginTop:20}}>
                    ...
                    <Text style={{fontFamily:'Chakra Petch SemiBold', fontSize:24, marginLeft:130, color:colors[this.context.theme.mode].text1}}>Profile</Text>
                </View>
                ...
                <View style={{width:Dimensions.get('window').width, flexDirection:'column', alignItems:'center', justifyContent:'flex-start'}}>
                    ...
                    <TouchableOpacity onPress={()=>{ !this.state.reqloading? this.profPicker() :'';}} style={{position:'relative', width:100, height:100, justifyContent:'center', alignItems:'center', marginTop:10, marginBottom:10}}>
                        <Image style={{width:100, height:100, borderRadius:'50%'}} source={{uri:/*this.state.userdata.profilepic?this.state.userdata.profilepic:*/'https://ppbe01.onrender.com/public/defaultpic.png'}}></Image>
                        <ActivityIndicator style={{display:this.state.reqloading?'flex':'none', position:'absolute'}}></ActivityIndicator>
                    </TouchableOpacity>
                    <Text style={{display:this.state.picchanged?'flex':'none', fontFamily:'Chakra Petch Regular', color:'green', textAlign:'center', width:Dimensions.get('window').width-40, marginBottom:10}}>
                    ...
                    <View style={{flexDirection:'row', alignItems:'center', justifyContent:'space-around', width:332, height:69, marginTop:10}}>
                        <View style={{flexDirection:'colum', alignItems:'center', justifyContent:'space-between', height:70}}>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:14, color:colors[this.context.theme.mode].text1}}>Games Created</Text>
                            <View style={{width:80, height:40, borderRadius:4, backgroundColor:colors[this.context.theme.mode].text1, alignItems:'center', justifyContent:'center'}}>
                                <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.context.theme.mode==='dark'?'black':'white', textAlign:'center', }}>{this.state.userdata.gamescreated?this.state.userdata.gamescreated.filter(x=>x.status!=='cancelled').length:'0'}</Text>
                            </View>
                        </View>

                        <View style={{flexDirection:'colum', alignItems:'center', justifyContent:'space-between', height:70}}>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:14, color:colors[this.context.theme.mode].text1}}>Bets Won</Text>
                            <View style={{width:80, height:40, borderRadius:4, backgroundColor:colors[this.context.theme.mode].text1, alignItems:'center', justifyContent:'center'}}>
                                <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.context.theme.mode==='dark'?'black':'white', textAlign:'center', }}>{this.state.userdata.gameswon?this.state.userdata.gameswon:'0'}</Text>
                            </View>
                        </View>

                        <View style={{flexDirection:'colum', alignItems:'center', justifyContent:'space-between', height:70}}>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:14, color:colors[this.context.theme.mode].text1}}>Bets Lost</Text>
                            <View style={{width:80, height:40, borderRadius:4, backgroundColor:colors[this.context.theme.mode].text1, alignItems:'center', justifyContent:'center'}}>
                                <Text style={{fontFamily:'Chakra Petch Regular', fontSize:18, color:this.context.theme.mode==='dark'?'black':'white', textAlign:'center', }}>{this.state.userdata.gameslost?this.state.userdata.gameslost:'0'}</Text>
                            </View>
                        </View>
                    </View>
                    ...
                    <View>
                        <View style={{marginTop:15}}>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:14, color:colors[this.context.theme.mode].text1}}>Full Name</Text>
                            <View style={{width:340, borderRadius:8, height:44, flexDirection:'row', alignItems:'center', justifyContent:'space-between', borderWidth:1, borderColor:colors[this.context.theme.mode].inputborder1, paddingLeft:15, paddingRight:15, marginTop:5}}>
                                <TextInput
                                    ...
                                    style={{outlineStyle:'none', width:320, borderRadius:8, height:44, fontSize:16, fontFamily:'Chakra Petch Regular', color:colors[this.context.theme.mode].text1}}
                                />
                        ...
                        <View style={{marginTop:15}}>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:14, color:colors[this.context.theme.mode].text1}}>Username</Text>
                            <View style={{width:340, borderRadius:8, height:44, flexDirection:'row', alignItems:'center', justifyContent:'space-between', borderWidth:1, borderColor:colors[this.context.theme.mode].inputborder1, paddingLeft:15, paddingRight:15, marginTop:5}}>
                                <TextInput
                        ...
                        <View style={{marginTop:15}}>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:14, color:colors[this.context.theme.mode].text1}}>Email</Text>
                            <View style={{width:340, borderRadius:8, height:44, flexDirection:'row', alignItems:'center', justifyContent:'space-between', borderWidth:1, borderColor:colors[this.context.theme.mode].inputborder1, paddingLeft:15, paddingRight:15, marginTop:5}}>
                                <TextInput
                        ...
                        <View style={{marginTop:15}}>
                            <Text style={{fontFamily:'Chakra Petch Regular', fontSize:14, color:colors[this.context.theme.mode].text1}}>Phone Number</Text>
                            <View style={{width:340, borderRadius:8, height:44, flexDirection:'row', alignItems:'center', justifyContent:'space-between', borderWidth:1, borderColor:colors[this.context.theme.mode].inputborder1, paddingLeft:15, paddingRight:15, marginTop:5}}>
                                <TextInput
                ...
                <View style={{width:Dimensions.get('window').width, flexDirection:'column', alignItems:'center', justifyContent:'flex-start', marginTop:40}}>
                    ...
                    <Text style={{fontFamily:'Chakra Petch Regular', fontSize:16, width:356, textAlign:'center', color:'', marginTop:10, color:colors[this.context.theme.mode].text1}}>{'Input the 6 digit code that was sent to '+this.state.email+'. If it is not in your inbox check your spam.'}</Text>
                    ...
                    <View style={{flexDirection:'column', alignItems:'center', justifyContent:'flex-start', marginTop:120}}>
                        ...
                        <TouchableOpacity style={{width:Dimensions.get('window').width-36, height:56, flexDirection:'row', alignItems:'center', justifyContent:'center', borderRadius:8, marginTop:20, backgroundColor:colors[this.context.theme.mode].btn1}} onPress={()=>{!this.state.emailloader?this.resendcode():'';}}>
